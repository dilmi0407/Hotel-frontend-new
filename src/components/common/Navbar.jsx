...
const Navbar = () => {
  ...
  return (
    <nav className="bg-blue-200 shadow-md sticky top-0 z-50 px-6 py-4">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
                ...
                <div className="relative px-4 py-2 rounded-lg group">
                  <span
                    className={`text-black text-lg font-bold transition-all duration-300 ${
                      isActive(link.href) ? "text-blue-800" : "group-hover:text-blue-700"
                    }`}
                  >
                    {link.label}
                  </span>
                  ...
                  {/* Hover effect */}
                  <div className="absolute inset-0 bg-blue-300 rounded-lg opacity-0 group-hover:opacity-20 transition-opacity duration-300" />
                </div>
              </Link>
            </motion.div>
          ))}
          ...
          {user ? (
            ...
            >
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-black text-lg font-bold hover:bg-blue-300/20 transition-colors duration-300 focus:outline-none"
              >
              ...
              <AnimatePresence>
                {isDropdownOpen && (
                  ...
                  >
                    {/*<Link
                      href="/pages/profile"
                      className="block px-4 py-3 text-sm text-gray-700 hover:bg-blue-100 transition-colors duration-200"
            ...
            >
              <Link
                href="/pages/auth"
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg transition-colors duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1 active:translate-y-0"
              >
                Login
              </Link>
            </motion.div>
        ...
        >
          <button
            ...
            className="focus:outline-none p-2 rounded-lg hover:bg-ablue-300/20 transition-colors duration-300"
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              ...
              className="md:hidden absolute top-24 right-0 left-0 bg-blue-200 shadow-lg z-10 overflow-hidden"
            >
              <div className="flex flex-col p-4 space-y-3 max-h-[calc(100vh-6rem)] overflow-y-auto">
                  ...
                  <Link key={link.href} href={link.href} onClick={() => setIsMenuOpen(false)}>
                    <div
                      className={`p-3 rounded-lg transition-all duration-300 ${
                        isActive(link.href) ? "bg-blue-300/40 font-bold" : "hover:bg-blue-300/20"
                      }`}
                    >
                      <span className="text-black text-lg font-bold">{link.label}</span>
                      {isActive(link.href) && <div className="h-1 w-12 bg-blue-500 mt-1 rounded-full" />}
                    </div>
                  </Link>
                ))}
                ...
                {user ? (
                  <>
                    <div className="py-3 border-t border-blue-300">
                      <p className="text-black font-medium">Hello, {user.name || user.email?.split("@")[0]}</p>
                    </div>
                    {/*<Link
                      href="/pages/profile"
                      className="p-3 rounded-lg text-black text-lg font-bold hover:bg-blue-300/20 transition-colors duration-300"
